...
from utils.gen6d_base_utils import read_pickle, save_pickle, pose_compose, load_point_cloud, pose_inverse, resize_img, \
    mask_depth_to_pts, transform_points_pose
from utils.gen6d_read_write_model import read_model
...
class LINEMODDatabase(BaseDatabase):
    ...
    def __init__(self, database_name):
        ...
        self.img_id2pose = {}
        self.root_dir = f'{LINEMOD_ROOT}/{self.model_name}'
    ...
    def _compute_depth_range(self):
        # if os.path.exists(f'{LINEMOD_ROOT}/{self.model_name}/depth_range.pkl'):
        #     self.range_dict=read_pickle(f'{LINEMOD_ROOT}/{self.model_name}/depth_range.pkl')
        # else:
        print('estimate depth range ...')
        self.range_dict={}
        self.depth_dict={}
        ...
        for img_id in self.img_ids:
            ...
            far = np.percentile(depth.flatten(),98)*1.2
            near = np.percentile(depth.flatten(),2)*0.8
            # far = depth.flatten().max()
            # near = depth.flatten().min()
            self.range_dict[img_id] = np.asarray([near,far], np.float32)
            self.depth_dict[img_id] = depth
        # save_pickle(self.range_dict, f'{LINEMOD_ROOT}/{self.model_name}/depth_range.pkl')

    def get_depth_range(self, img_id):
        return self.range_dict[img_id].copy()

    def get_depth(self, img_id):
        depth = read_array(f'{LINEMOD_ROOT}/{self.model_name}/dense/stereo/depth_maps/{int(img_id):06}.jpg.geometric.bin').astype(np.float32)
        return depth
