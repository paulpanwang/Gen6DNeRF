...
def select_train_ids_for_real_estate(img_ids):
    ...
    id_feat = np.random.choice(candidate_ids,
                               size=min(8, len(candidate_ids)),
                               replace=False)
    img_ids = np.asarray(img_ids)
    return img_ids[id_render], img_ids[id_feat]


def add_depth_offset(depth, mask, region_min, region_max, offset_min,
                     offset_max, noise_ratio, depth_length):
    ...
    local_offset = np.random.uniform(-noise_ratio, noise_ratio, masked_coords.
                                     shape[0]) * depth_length + global_offset
    depth[masked_coords[:, 1], masked_coords[:, 0]] += local_offset


def build_src_imgs_info_select(database,
                               ref_ids,
                               ref_ids_all,
                               cost_volume_nn_num,
                               pad_interval=-1):
    # ref_ids - selected ref ids for rendering
    ref_idx_exp = compute_nearest_camera_indices(database, ref_ids,
                                                 ref_ids_all)
    ...
    mask1 = ref_ids_in[None, :] == ref_ids_exp.flatten()[:,
                                                         None]  # nn*rfn,rfn'
    ...
    ref_imgs_info = build_imgs_info(database, ref_ids_in, pad_interval,
                                    is_aligned)
...
class GeneralRendererDataset(Dataset):
    default_cfg = {
        'train_database_types':
        ['dtu_train', 'space', 'real_iconic', 'real_estate', 'gso'],
        'type2sample_weights': {
            'gso': 20,
            'dtu_train': 20,
            'real_iconic': 20,
        ...
        'val_database_name':
        'nerf_synthetic/lego/black_800',
        'val_database_split_type':
        'val',
        'min_wn':
        8,
        'max_wn':
        ...
        "cost_volume_nn_num":
        3,
        "aug_gso_shrink_range_prob":
        0.5,
        "aug_depth_range_prob":
        0.05,
        'aug_depth_range_min':
        0.95,
        'aug_depth_range_max':
        1.05,
        "aug_use_depth_offset":
        True,
        "aug_depth_offset_prob":
        0.25,
        "aug_depth_offset_region_min":
        0.05,
        "aug_depth_offset_region_max":
        0.1,
        'aug_depth_offset_min':
        0.5,
        'aug_depth_offset_max':
        1.0,
        'aug_depth_offset_local':
        0.1,
        "aug_use_depth_small_offset":
        True,
        "aug_use_global_noise":
        True,
        "aug_global_noise_prob":
        0.5,
        "aug_depth_small_offset_prob":
        0.5,
        "aug_forward_crop_size": (400, 600),
        "aug_pixel_center_sample":
        False,
        "aug_view_select_type":
        "easy",
        "use_consistent_min_max":
        False,
        "revise_depth_range":
        False,
    }
    ...
    def __init__(self, cfg, is_train):
        ...
        if is_train:
            ...
            if self.cfg['resolution_type'] == 'hr':
                type2scene_names = {
                    'dtu_train': dtu_train_scene_names,
                    'space': space_scene_names,
                    'real_iconic': real_iconic_scene_names_4,
                    'real_estate': real_estate_scene_names,
                    'gso': gso_scene_names
                }
                ...
                type2scene_names = {
                    ...
                    'real_estate': real_estate_scene_names,
                    'gso': gso_scene_names_400
                }
            else:
                raise NotImplementedError

            for database_type in self.cfg['train_database_types']:
                self.type2scene_names[database_type] = type2scene_names[
                    database_type]
                self.database_types.append(database_type)
                self.database_weights.append(
                    self.cfg['type2sample_weights'][database_type])
            assert (len(self.database_types) > 0)
            # normalize weights
            self.database_weights = np.asarray(self.database_weights)
            self.database_weights = self.database_weights / np.sum(
                self.database_weights)
        else:
            self.database = parse_database_name(self.cfg['val_database_name'])
            self.ref_ids, self.que_ids = get_database_split(
                self.database, self.cfg['val_database_split_type'])
    ...
    def get_database_ref_que_ids(self, index):
        if self.is_train:
            database_type = np.random.choice(self.database_types,
                                             1,
                                             False,
                                             p=self.database_weights)[0]
            database_scene_name = np.random.choice(
                self.type2scene_names[database_type])
            ...
            while True:
                ...
                if len(ref_ids) == 0:
                    database_type = np.random.choice(self.database_types, 1,
                                                     False,
                                                     self.database_weights)[0]
                    database_scene_name = np.random.choice(
                        self.type2scene_names[database_type])
                    database = parse_database_name(database_scene_name)
                else:
    ...
    def select_working_views_impl(self, database_name, dist_idx, ref_num):
        if self.cfg['aug_view_select_type'] == 'default':
            if database_name.startswith('space') or database_name.startswith(
                    'real_estate'):
                pass
    ...
    def select_working_views(self, database, que_id, ref_ids):
        ...
        dist_idx = compute_nearest_camera_indices(database, [que_id],
                                                  ref_ids)[0]
        if self.is_train:
            ...
            dist_idx = self.select_working_views_impl(database_name, dist_idx,
                                                      ref_num)
    ...
    def depth_range_aug_for_gso(self, depth_range, depth, mask):
        ...
        if np.random.random() < self.cfg['aug_gso_shrink_range_prob']:
            ...
            if far_ratio < 1.0:
                depth_range_new[:, 1] *= np.random.uniform(far_ratio, 1.0)
            if near_ratio < 1.0:
                depth_range_new[:, 0] /= np.random.uniform(near_ratio, 1.0)
    ...
    def random_change_depth_range(self, depth_range, depth, mask,
                                  database_name):
        if database_name.startswith('gso'):
            depth_range_new = self.depth_range_aug_for_gso(
                depth_range, depth, mask)
        else:
            depth_range_new = depth_range.copy()
            if np.random.random() < self.cfg['aug_depth_range_prob']:
                depth_range_new[:, 0] *= np.random.uniform(
                    self.cfg['aug_depth_range_min'], 1.0)
                depth_range_new[:, 1] *= np.random.uniform(
                    1.0, self.cfg['aug_depth_range_max'])
        return depth_range_new
    ...
    def add_depth_noise(self, depths, masks, depth_ranges):
        ...
        depths_output = []
        for rfi in range(rfn):
            depth, mask, depth_range = depths[rfi,
                                              0], masks[rfi,
                                                        0], depth_ranges[rfi]
            ...
            if self.cfg['aug_use_depth_offset'] and np.random.random(
            ) < self.cfg['aug_depth_offset_prob']:
                add_depth_offset(depth, mask,
                                 self.cfg['aug_depth_offset_region_min'],
                                 self.cfg['aug_depth_offset_region_max'],
                                 self.cfg['aug_depth_offset_min'],
                                 self.cfg['aug_depth_offset_max'],
                                 self.cfg['aug_depth_offset_local'],
                                 depth_length)
            if self.cfg['aug_use_depth_small_offset'] and np.random.random(
            ) < self.cfg['aug_depth_small_offset_prob']:
                add_depth_offset(depth, mask, 0.1, 0.2, 0.01, 0.05, 0.005,
                                 depth_length)
            if self.cfg['aug_use_global_noise'] and np.random.random(
            ) < self.cfg['aug_global_noise_prob']:
                depth += np.random.uniform(-0.005, 0.005, depth.shape).astype(
                    np.float32) * depth_length
    ...
    def generate_coords_for_training(self, database, que_imgs_info):
        if (database.database_name.startswith('real_estate') \
            ...
            que_mask_cur = np.zeros_like(que_imgs_info['masks'][0, 0]).astype(
                np.bool)
            h, w = que_mask_cur.shape
            center_ratio = 0.8
            begin_ratio = (1 - center_ratio) / 2
            hb, he = int(h * begin_ratio), int(h *
                                               (center_ratio + begin_ratio))
            wb, we = int(w * begin_ratio), int(w *
                                               (center_ratio + begin_ratio))
            que_mask_cur[hb:he, wb:we] = True
            coords = get_coords_mask(que_mask_cur, self.cfg['train_ray_num'],
                                     0.9).reshape([1, -1, 2])
        else:
            que_mask_cur = que_imgs_info['masks'][0, 0] > 0
            coords = get_coords_mask(que_mask_cur, self.cfg['train_ray_num'],
                                     self.cfg['foreground_ratio']).reshape(
                                         [1, -1, 2])
        return coords

    def consistent_depth_range(self, ref_imgs_info, que_imgs_info):
        depth_range_all = np.concatenate(
            [ref_imgs_info['depth_range'], que_imgs_info['depth_range']], 0)
        if self.cfg['use_consistent_min_max']:
            ...
            ref_near = np.max(
                np.stack([ref_near, depth_range_all[:, 0] * 0.5], -1), 1)
    ...
    def __getitem__(self, index):
        ...
        if self.cfg['use_src_imgs']:
            ...
            ref_imgs_info, ref_cv_idx, ref_real_idx = build_src_imgs_info_select(
                database, ref_ids, ref_ids_all, self.cfg['cost_volume_nn_num'])
        else:
            ref_idx = compute_nearest_camera_indices(
                database, ref_ids)[:, 1:4]  # used in cost volume construction
            is_aligned = not database.database_name.startswith('space')
            ref_imgs_info = build_imgs_info(database, ref_ids, -1, is_aligned)
        que_imgs_info = build_imgs_info(database, [que_id],
                                        has_depth=self.is_train)

        if self.is_train:
            # data augmentation
            depth_range_all = np.concatenate(
                [ref_imgs_info['depth_range'], que_imgs_info['depth_range']],
                0)
            if database.database_name.startswith(
                    'gso'):  # only used in gso currently
                depth_all = np.concatenate(
                    [ref_imgs_info['depth'], que_imgs_info['depth']], 0)
                mask_all = np.concatenate(
                    [ref_imgs_info['masks'], que_imgs_info['masks']], 0)
            else:
                depth_all, mask_all = None, None
            depth_range_all = self.random_change_depth_range(
                depth_range_all, depth_all, mask_all, database.database_name)
            ref_imgs_info['depth_range'] = depth_range_all[:-1]
            que_imgs_info['depth_range'] = depth_range_all[-1:]

            if database.database_name.startswith(
                    'gso') and self.cfg['use_depth']:
            ...
            if database.database_name.startswith('real_estate') \
                ...
                # crop all datasets
                ref_imgs_info, que_imgs_info = random_crop(
                    ref_imgs_info, que_imgs_info,
                    self.cfg['aug_forward_crop_size'])
                if np.random.random() < 0.5:
                    ref_imgs_info, que_imgs_info = random_flip(
                        ref_imgs_info, que_imgs_info)
        ...
        ref_imgs_info = pad_imgs_info(ref_imgs_info,
                                      self.cfg['ref_pad_interval'])
        ...
        outputs = {
            ...
            'scene_name': database.database_name
        }
        if self.cfg['use_src_imgs']:
...
class FinetuningRendererDataset(Dataset):
    ...
    def __init__(self, cfg, is_train):
        self.cfg = {**self.default_cfg, **cfg}
        self.is_train = is_train
        self.train_ids, self.val_ids = get_database_split(
            parse_database_name(self.cfg['database_name']),
            self.cfg['database_split'])
