...
from dataset.gen6d_database import get_object_center, get_diameter, get_object_vert
from utils.gen6d_base_utils import *
from utils.gen6d_pose_utils import scale_rotation_difference_from_cameras, let_me_look_at, let_me_look_at_2d
...
def normalize_reference_views(database, ref_ids, size, margin,
    ...
    for k in range(len(ref_ids)):
        ...
        try:
            ref_depth = database.get_depth(ref_ids[k]).astype(np.float32)
            ref_depth_new.append(cv2.warpPerspective(ref_depth, ref_H, (size, size), flags=cv2.INTER_LINEAR))
        except:
            continue
        ...
        ref_mask_new = cv2.warpPerspective(ref_mask, ref_H, (size, size), flags=cv2.INTER_LINEAR)
        ref_masks_new.append(ref_mask_new)

        objmask = np.stack([ref_mask_new, ref_mask_new, ref_mask_new], axis=2)
        ref_masked_img = np.uint8(ref_img_new*(objmask))
        ref_imgs_new.append(ref_masked_img)
    ...
    ref_depth_new = np.asarray(ref_depth_new, dtype=np.float32)[:, None, :, :]
    if add_rots:
        ref_imgs_rots = np.stack(ref_imgs_rots,1)
        return ref_imgs_new, ref_masks_new, ref_Ks_new, ref_poses_new, ref_Hs, ref_imgs_rots
    return ref_imgs_new, ref_masks_new, ref_Ks_new, ref_poses_new, ref_Hs, ref_depth_new
