...
class INeRFLoss:
    ...
    def __call__(self,  render_image_rgb, target_image_rgb ,render_image_fea, target_image_fea):
        loss_vit = F.mse_loss( render_image_fea,target_image_fea) 
        loss_mse = F.mse_loss( render_image_rgb/255., target_image_rgb/255.)
        loss_dict =  {
            "loss_vit": loss_vit.item(),
            "lose_mse": loss_mse.item()
        }
        print("loss:", loss_dict)
        return  loss_vit + self.weight*loss_mse
