...
from utils.gen6d_draw_utils import concat_images_list
...
class NeuralRayBaseRenderer(nn.Module):
    ...
    # get the feature of reference /query image
    ...
    def render(self, que_imgs_info, ref_imgs_info, is_train):
        ...
        # step1 get the feature of ref img
...
class NeuralRayGenRenderer(NeuralRayBaseRenderer):
    ...
    def gen_depth_loss_coords(self,h,w,device):
        coords = torch.stack(torch.meshgrid(torch.arange(h), torch.arange(w),indexing='ij'), -1).reshape(-1, 2).to(device)
...
from utils.eval_time import get_time_second
class NeuralRayGen6DRenderer(NeuralRayGenRenderer):    
    default_cfg={
        'init_net_type': 'depth',
        'init_net_cfg': {},
        'use_depth_loss': True,
    ...
    def __init__(self,cfg):
        cfg = {**self.default_cfg,**cfg}
        super().__init__(cfg)
        self.fea_ext = VitExtractor(model_name='dino_vits16')
        print("[debug]: dino model is loaded")
        self.init_net = name2init_net[self.cfg['init_net_type']](self.cfg['init_net_cfg'])
    ...
    def forword_6dpose( self, data,  target_image ):
        ref_imgs_info = data['ref_imgs_info'].copy()
        que_imgs_info = data['que_imgs_info'].copy()
        is_train = 'eval' not in data
        src_imgs_info = data['src_imgs_info'].copy() if 'src_imgs_info' in data else None
        ...
        if (self.cfg['use_depth_loss'] and 'true_depth' in ref_imgs_info) or (not is_train):
            render_outputs.update(self.predict_mean_for_depth_loss(ref_imgs_info))
        render_rgb = render_outputs["pixel_colors_nr_fine"]
        h, w = data.get("que_shape")
        # print("to(render_rgb.device):" , to(render_rgb.device)
        
        target_image_copy = target_image.clone().reshape(1,h,w ,3)
        target_image_copy = torch.permute(target_image_copy, (0, 3, 1, 2))
        render_rgb_copy = render_rgb.clone().reshape(1,h,w,3)
        render_rgb_copy = torch.permute(render_rgb_copy, (0, 3, 1, 2))
        target_image_copy = target_image_copy.to(render_rgb_copy.device)
        
        self.fea_ext = self.fea_ext.to(render_rgb.device)
        target_image_fea = self.fea_ext.get_vit_feature(target_image_copy)

        render_image_fea = self.fea_ext.get_vit_feature(render_rgb_copy)
        results = (render_rgb_copy, target_image_copy, render_image_fea, target_image_fea)
        return  render_rgb, results
...
# 通过字典定义想要的网络
name2network={
    'neuray_gen6d': NeuralRayGen6DRenderer,
