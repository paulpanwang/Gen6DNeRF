...
def depth2pts3d(depth, ref_Ks, ref_poses):
    ...
    pts3d = torch.inverse( ref_Ks.float() ) @ pts3d  # rfn
    R = ref_poses[:, :, :3].permute(0, 2, 1).float()  # rfn,3,3
    t = -R @ ref_poses[:, :, 3:].float()  # rfn,3,1
...
class DepthInitNet(nn.Module):
    ...
    def forward(self, ref_imgs_info, src_imgs_info, is_train):
        ...
        # print( type(imgs),  type(depth), type(diff_feats) )
