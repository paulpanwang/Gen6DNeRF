...
class SphericalHarmonicsSolver(nn.Module):
    ...
    def __init__(self, degree=3, init_regs=(0.001, 0.005, 0.05, 0.1)):
        ...
        regs = [
            np.zeros([1]),
            np.ones([1 * 2 + 1]) * init_regs[0],
            np.ones([2 * 2 + 1]) * init_regs[1],
            np.ones([3 * 2 + 1]) * init_regs[2],
            np.ones([4 * 2 + 1]) * init_regs[3]
        ]
        regs = torch.from_numpy(
            np.concatenate(regs[:degree + 1], 0).astype(np.float32))
    ...
    def get_coefficient_func():
        ...
        l2 = lambda x, y, z: torch.stack(
            [x * y, y * z, -x**2 - y**2 + 2 * z**2, z * x, x**2 - y**2], -1)
        l3 = lambda x, y, z: torch.stack([(3 * x**2 - y**2) * y, x * y * z, y *
                                          (4 * z**2 - x**2 - y**2), z *
                                          (2 * z**2 - 3 * x**2 - 3 * y**2), x *
                                          (4 * z**2 - x**2 - y**2),
                                          (x**2 - y**2) * z,
                                          (x**2 - 3 * y**2) * x], -1)
        l4 = lambda x, y, z: torch.stack([
            x * y * (x**2 - y**2), (3 * x**2 - y**2) * y * z, x * y *
            (7 * z**2 - 1), y * z *
            (7 * z**2 - 3), 35 * z**4 - 30 * z**2 + 3, x * z * (7 * z**2 - 3),
            (x**2 - y**2) * (7 * z**2 - 1), (x**2 - 3 * y**2) * x * z, x**2 *
            (x**2 - 3 * y**2) - y**2 * (3 * x**2 - y**2)
        ], -1)
